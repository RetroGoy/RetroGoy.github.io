...
document.addEventListener('DOMContentLoaded', () => {
    const inputInstrument = document.getElementById("input-instrument");
    const outputInstrument = document.getElementById("output-instrument");
    ...
    const showPentatonic = document.getElementById("show-pentatonic");
    ...
    translateButton.addEventListener("click", () => {
        const input = inputInstrument.value;
        const output = outputInstrument.value;
        const key = keySelection.value;
        const intervals = showIntervals.checked;
        const pentatonic = showPentatonic.checked;
        const style = musicStyle.value;

        let translatedNotes = `Traduction de ${input} à ${output} en ${key}\n`;
        translatedNotes += `Style: ${style}\n`;

        if (intervals) {
            translatedNotes += "Afficher les intervalles\n";
        }

        if (pentatonic) {
            translatedNotes += "Afficher les échelles pentatoniques\n";
        }

        translatedNotes += "Notes traduites : Do, Ré, Mi, Fa, Sol, La, Si";

        outputNotes.textContent = translatedNotes;
    });
...
const bassMapping = {
    ...
    'F2': [{ string: 'E', fret: 13 }, { string: 'A', fret: 8 }, { string: 'D', fret: 3 }],
...
let lastPlayedPosition = { string: 'E', fret: 0 };
...
function updateBassDisplay(position) {
    ...
    const stringDiv = document.querySelector(`.bass .corde.${position.string} .string:nth-child(${position.fret + 1})`);
    stringDiv.style.backgroundColor = 'red';

    const tabString = tabStrings[position.string];
    alignTablature();
    tabString.textContent += position.fret + "-";
    alignTablature();
}
